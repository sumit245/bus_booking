...
if ($request->DateOfJourney && Carbon::parse($request->DateOfJourney)->format('Y-m-d') < Carbon::now()->format('Y-m-d'))
    ...
    $resp = searchAPIBuses($request->ip(), $request->OriginId, $request->DestinationId,
    ...
    // if ($startPoint < $endPoint) { // if ($pickup_point>= $startPoint && $pickup_point < $endPoint &&
            $destination_point> $startPoint && $destination_point <= $endPoint) { // array_push($tripArray, $trip->id);
                // }
                // } else {
                ...
                // if ($pickup_point >= $startPoint && $pickup_point < $endPoint && $destination_point> $startPoint &&
                    $destination_point <= $endPoint) { // array_push($tripArray, $trip->id);
                        // }
                        // }
                        // }
                        ...
                        // $trips = $trips->with(['fleetType', 'route', 'schedule', 'startFrom',
                        'endTo'])->where('status', 1)->paginate(getPaginate());
                        ...
                        //FIXME print_r($resp);
                        return view($this->activeTemplate . 'ticket', compact('pageTitle', 'fleetType', 'trips',
                        'routes', 'schedules', 'emptyMessage', 'layout'));
                        }