...
@section('panel')
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title mb-0">@lang('Manage Dropping Points') -
                        {{ $schedule->schedule_name ?: 'Schedule #' . $schedule->id }}</h4>
                    <div class="card-tools">
                        <button type="button" class="btn btn--success btn-sm" data-toggle="modal" data-target="#addPointModal">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-3">
                            <div class="info-item">
                                <label>@lang('Arrival Time')</label>
                                <p><span class="badge badge--success">{{ $schedule->formatted_arrival_time }}</span></p>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="info-item">
                    ...
                    <!-- Schedule-Specific Dropping Points -->
                    <div class="row mb-4">
                        <div class="col-12">
                            <h5 class="mb-3">@lang('Schedule-Specific Dropping Points') ({{ $schedule->droppingPoints->count() }})</h5>
                            <div class="table-responsive">
                                <table class="table table--light style--two">
                                    ...
                                    <tbody>
                                        @forelse($schedule->droppingPoints as $point)
                                            <tr>
                                                <td><span class="badge badge--warning">{{ $point->point_index }}</span></td>
                                                ...
                                                <td><span
                                                        class="badge badge--success">{{ \Carbon\Carbon::parse($point->point_time)->format('h:i A') }}</span>
                                                </td>
                                                <td>
                                                    @if ($point->status)
                                                        <span class="badge badge--success">@lang('Active')</span>
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="9" class="text-center text-muted">@lang('No schedule-specific dropping points found. Add points or route-level points will be used as fallback.')</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
                    ...
                    <!-- Route-Level Dropping Points (Fallback) -->
                    @if ($schedule->operatorRoute->droppingPoints->count() > 0)
                        <div class="row">
                            <div class="col-12">
                                <div class="alert alert--info">
                                    <i class="las la-info-circle"></i>
                                    @lang('Route-level dropping points below will be used as fallback if no schedule-specific points are defined.')
                                </div>
                                <h5 class="mb-3">@lang('Route-Level Dropping Points (Fallback)')
                                    ({{ $schedule->operatorRoute->droppingPoints->count() }})</h5>
                                <div class="table-responsive">
                                    <table class="table table--light style--two">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            @foreach ($schedule->operatorRoute->droppingPoints as $point)
    ...
    <div class="modal fade" id="addPointModal" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <form method="POST" action="{{ route('operator.schedules.dropping-points.store', $schedule) }}">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title">@lang('Add Dropping Point')</h5>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label>@lang('Point Address')</label>
                                    <textarea name="point_address" class="form-control" rows="2"></textarea>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <small class="text-muted">@lang('Drop-off time at this point')</small>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn--success">@lang('Add Point')</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <form method="POST" id="editForm">
                    ...
                    @method('PATCH')
                    <div class="modal-header">
                        <h5 class="modal-title">@lang('Edit Dropping Point')</h5>
                        <button type="button" class="close" data-dismiss="modal">
                            <span>&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label>@lang('Point Address')</label>
                                    <textarea name="point_address" id="edit_point_address" class="form-control" rows="2"></textarea>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn--secondary"
                            data-dismiss="modal">@lang('Cancel')</button>
                        <button type="submit" class="btn btn--success">@lang('Update Point')</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Delete Dropping Point')</h5>
                    <button type="button" class="close" data-dismiss="modal">
                        <span>&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <p>@lang('Are you sure you want to delete this dropping point? This action cannot be undone.')</p>
                </div>
                <div class="modal-footer">
...
@push('breadcrumb-plugins')
    <a href="{{ route('operator.schedules.show', $schedule) }}" class="btn btn-sm btn--success box--shadow1 text--small">
...
@push('script')
    <script>
        function editPoint(id, name, address, location, landmark, contact, index, time, status) {
            ...
            document.getElementById('editForm').action = `/operator/schedules/{{ $schedule->id }}/dropping-points/${id}`;
            $('#editPointModal').modal('show');
        }

        function deletePoint(pointId) {
            document.getElementById('deleteForm').action =
                `/operator/schedules/{{ $schedule->id }}/dropping-points/${pointId}`;
