<div class="bus">{!! renderSeatHTML($seatHtml) !!}</div>
      ...
      setTimeout(() => {
        ...
        for (let i = 0; i < sortedTops.length; i++) {
          ...
          for (const {
            ...
            const match = onclick.match(/AddRemoveSeat\(this,\s*['"]([^'"]+)['"]\s*,\s*['"]([^'"]+)['"]\)/);
            if (!match) continue;
        ...
        seatContainers.forEach(deck => {
          ...
          Object.entries(result).forEach(([rowName, seats]) => {
            ...
            seats.sort((a, b) => a.left - b.left).forEach(({
              ...
              const seat = [...deck.children].find(el => el.classList.contains(rowName) && !rowDiv
                .contains(el));
