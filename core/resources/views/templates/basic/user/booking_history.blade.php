                        ...
                        <tbody>
                            @forelse ($bookedTickets as $item)
                                ...
                                <tr>
                                    ...
                                    <td class="fare" data-label="Fare">
                                        {{ __(showAmount($item->total_amount > 0 ? $item->total_amount : $item->sub_total)) }}
...
@push('script')
    <script>
        ...
        $('.checkinfo').on('click', function() {
            ...
            if (info.bus_details) {
                try {
                    busDetails = typeof info.bus_details === 'object' ? info.bus_details : JSON.parse(info
                        .bus_details);
                } catch (e) {
            ...
            if (info.boarding_point_details) {
                try {
                    boardingPointDetails = typeof info.boarding_point_details === 'object' ? info
                        .boarding_point_details : JSON.parse(info.boarding_point_details);
                } catch (e) {
            ...
            if (info.dropping_point_details) {
                try {
                    droppingPointDetails = typeof info.dropping_point_details === 'object' ? info
                        .dropping_point_details : JSON.parse(info.dropping_point_details);
                } catch (e) {
            ...
            if (busDetails && busDetails.departure_time) {
                departureTime = new Date(busDetails.departure_time).toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit'
                });
            } else if (info.departure_time && info.departure_time !== '00:00:00') {
                departureTime = new Date('2000-01-01T' + info.departure_time).toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit'
                });
            }
            ...
            if (busDetails && busDetails.arrival_time) {
                arrivalTime = new Date(busDetails.arrival_time).toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit'
                });
            } else if (info.arrival_time && info.arrival_time !== '00:00:00') {
                arrivalTime = new Date('2000-01-01T' + info.arrival_time).toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit'
                });
            }
            ...
            var pickupName = boardingPointDetails ? boardingPointDetails.CityPointName : (info.pickup ? info.pickup
                .name : 'N/A');
            var droppingName = droppingPointDetails ? droppingPointDetails.CityPointName : (info.drop ? info.drop
                .name : 'N/A');
            ...
            <hr class="my-2">
            <p class="d-flex flex-wrap justify-content-between"><strong>@lang('Base Fare')</strong>  <span>${parseInt(info.sub_total).toFixed(2)} {{ __($general->cur_text) }}</span></p>
            ${info.service_charge > 0 ? `<p class="d-flex flex-wrap justify-content-between"><strong>@lang('Service Charge')</strong>  <span>${parseFloat(info.service_charge).toFixed(2)} {{ __($general->cur_text) }}</span></p>` : ''}
            ${info.platform_fee > 0 ? `<p class="d-flex flex-wrap justify-content-between"><strong>@lang('Platform Fee')</strong>  <span>${parseFloat(info.platform_fee).toFixed(2)} {{ __($general->cur_text) }}</span></p>` : ''}
            ${info.gst > 0 ? `<p class="d-flex flex-wrap justify-content-between"><strong>@lang('GST')</strong>  <span>${parseFloat(info.gst).toFixed(2)} {{ __($general->cur_text) }}</span></p>` : ''}
            <hr class="my-2">
            <p class="d-flex flex-wrap justify-content-between"><strong>@lang('Total Amount')</strong>  <span class="text-primary fw-bold">${parseFloat(info.total_amount || info.sub_total).toFixed(2)} {{ __($general->cur_text) }}</span></p>
